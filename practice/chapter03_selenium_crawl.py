...
# : 웹드라이버에서 페이지가 완전히 로딩되기 전에 page_source를 가져오기 때문에 미완성된 코드로 내용을 수집하는데 한계가 있음.
#   그래서 5초간 시간을 주고 페이지가 전부 로딩되면 그때 소스를 가져오도록 하기 위함.
time.sleep(5) # 5초간 waiting

page_code = driver.page_source # 해당 URL의 전체 소스코드 가져오기
print(page_code)